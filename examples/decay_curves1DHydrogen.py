...
from SemiGlobal.FourierGrid import Hpsi
...
def Gdiff_op(u1, t1, u2, t2):
    return (
        ...
        * 0.1
        * (
            np.cos(0.06 * (t1 - 500)) / np.cosh((t1 - 500) / (170)) ** 2
            - np.cos(0.06 * (t2 - 500)) / np.cosh((t2 - 500) / (170)) ** 2
        )
        * u1
    )
...
def errorSGarticle(T, Nt_ts, Nkr, minNt, Nsamp, Niter_input=1):
    ...
    for degi in range(0, Nsamp):
        ...
        er_decay["aller"][degi] = norm(u[:, 1] - Mdata["Uex"][:, -1]) / norm(
            Mdata["Uex"][:, -1]
        )
    plt.figure()
