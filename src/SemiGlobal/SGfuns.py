...
def r2Taylor4(
    x: np.ndarray, Dsize: int
):  # Computation of conversion coefficients from Newton basis polynomials to Taylor polynomials
    """
    ...
    -------
    np.ndarray
        2D ndarray which contains the conversion coefficients; the row index
    ...
    Dfactor = 4 / Dsize
    NC = x.size
    output_type = (x[0] + 0.0).dtype.type
    ...
    for ri in range(1, NC):
        ...
        for Taylori in range(1, ri):
            Q[ri, Taylori] = (
                Q[ri - 1, Taylori - 1] - x[ri - 1] * Q[ri - 1, Taylori]
            ) * Dfactor
...
def Cpoly2Ctaylor(
    ...
    ----------
    Cpoly: np.ndarray
        2D ndarray; contains the vector coefficients of the polynomial set in separate columns.
    Cp2t: np.ndarray
        2D ndarray; contains the conversion coefficients from the
    ...
    for polyi in range(1, NC):
        Ctaylor[:, 0 : (polyi + 1)] += (
            Cpoly[:, polyi][:, np.newaxis] * Cp2t[polyi, 0 : (polyi + 1)]
        )
    return Ctaylor


def guess0(ui: np.ndarray, Np: int):
    """
...
def maketM(
    t: np.ndarray, Nt_ts: int
):  # Computation of transpose Vandermonde matrix of time points
    """
    Computation of the matrix of time Taylor polynomials. It is equivalent to the
    transpose Vandermonde matrix for the time points specified by t.

    Parameters
    ----------
    t: np.ndarray
        1D ndarray containing the time-points
    Nt_ts: int
        The number of rows, which is the number of time sampling points in the
...
def f_fun(
    ...
    tol: float,
    factorialNt_ts: int,
):  # Computation of the function \tilde{f}_{Nt_ts}(z, t)
    """
    ...
    ----------
    z: np.ndarray
        1D ndarray with the required points in the eigenvalue domain of \tilde{G}
    t: np.ndarray
        1D ndarray with the required time-points
    Nt_ts: int
        The number of interior time-points in the time-step; specifies the
        computed function :math:`\tilde{f}_{Nt_ts}(z, t)`.
    tol: float The required tolerance of the computation
    factorialNt_ts: int
        The value of Nt_ts!; required for the error estimation.

    Returns
    -------
    np.ndarray
        2D ndarray with the function values; the row index indexes different
    ...
    output_type = (zt[0, 0] + 0.0).dtype.type
    ...
    while is_not_converged.max():
        term[is_not_converged] = (
            zt[is_not_converged] * term[is_not_converged] / (polydeg + Nt_ts)
        )
        result[is_not_converged] += term[is_not_converged]
        polydeg += 1
        is_not_converged[is_not_converged] = np.abs(term[is_not_converged]) / np.abs(
            result[is_not_converged]
        ) > np.spacing(1)
...
def f_chebC(
    ...
    """
    The function computes the Chebyshev coefficients of :math:`\tilde{f}_{Nt_ts}(z, t)`,
    where z is the argument of the Chebyshev expansion, and t serves as a parameter.

    Parameters
    ----------
    t: np.ndarray
        1D ndarray of time-values
    Nz: np.ndarray
        The number of Chebyshev sampling points for the z argument
    Nt_ts: int
        Defines the function as above
    leftb: float
        The minimum boundary of the approximation domain
    rightb: float
        The maximum boundary of the approximation domain
    tol: float
        The tolerance parameter for the computation of :math:`\tilde{f}_{Nt_ts}(z, t)`

    Returns
    -------
        2D ndarray containing the Chebyshev coefficients, where different
...
def Ufrom_vCheb(
    ...
    ----------
    v_tpols: np.ndarray
        2D ndarray; The v_j vectors excluding the last one, :math:`j=0,1,...,Nt_ts-1`, in
        seperate columns; the vector coefficients of the Taylor time-polynomials
        in the solution equation
    timeM: np.ndarray
        2D ndarray; represents the matrix of the t powers for the required time-points
    Vcheb: np.ndarray
        2D ndarray; the :math:`T_k(\tilde{G})v_{Nt_ts}` vectors, :math:`k=0,1,...,Nt_ts-1`, in
        seperate columns
    Ccheb_f: np.ndarray
        2D ndarray; the Chebyshev coefficients of :math:`\tilde{f}_{Nt_ts}(z, t)` in the
        required time-points specified by `timeM`, as computed by the function `f_chebC`
    f_error: float
        The estimated relative error of the computation of
        :math:`\tilde{f}_{Nt_ts}(G,t[Nt_ts - 1])v_vecs[:, Nkr]`;
...
def Ufrom_vArnoldi(
    v_tpols: np.ndarray,
    timeM: np.ndarray,
    Upsilon_without_bar: np.ndarray,
    RvKr: np.ndarray,
    ...
    ----------
    v_tpols: np.ndarray
        2D ndarray; The columns are the the v_j vectors excluding the last one,
        :math:`j=0,1,...,Nt_ts-1`; the vector coefficients of the Taylor time-polynomials in
        the solution equation
    timeM: np.ndarray
        2D ndarray; represents the matrix of the t powers for the required
        time-points (see the function maketM).
    Upsilon_without_bar: np.ndarray
        2D ndarray, containing the orthonormalized Krylov space
        vectors which participate in the approximation
    RvKr: np.ndarray
        2D ndarray of the vectors computed by Arnoldi.getRvKr
    samplingp: np.ndarray
        The sampling points for the Newton expansion in the Krylov space
    capacity: float
        The capacity of the approximation domain
    Nt_ts: int
        Number of internal Chebyshev time-points
    tol: float
        The tolerance parameter for computation of :math:`\tilde{f}_{Nt_ts}(z, t)`
    factorialNt_ts: float
        The value of Nt_ts!
    estimate_error: bool default False
        True means that the error of the computation of
        :math:`\tilde{f}_{Nt_ts}(G,t[Nt_ts - 1])v_vecs[:, Nkr]` and the resulting error of U is
        required and is to be returned. Otherwise, estimate_error=False.

    Returns
    -------
    Union[np.ndarray,Tuple[np.ndarray,float,float]]
        U: 2D ndarray; the solution in the required time points, where the different or
        time-points are represented by seperate columns if `estimate_error == False` else if
        `estimate_error == True` a tuple there second and third items:
        `f_error`: The estimated relative error of the computation of
        :math:`\tilde{f}_{Nt_ts}(G,t[Nt_ts - 1])v_vecs[:, Nkr]`;
        `fUerror`: The estimated relative error of U resulting from the computational
        error of :math:`\tilde{f}_{Nt_ts}(\tilde{G},t[Nt_ts - 1])v_vecs[:, Nkr]`;
    ...
    fdvd_ts, _ = divdif(
        samplingp / capacity, f_fun(samplingp, tp, Nt_ts, tol, factorialNt_ts).T
    )
    fdvd_ts = fdvd_ts.T
...
class GopFunction(Protocol):
    def __call__(
        ...
        *args,
        ...
        """A function object of the form

        Parameters
        ----------
        u : np.ndarray
            A 1D ndarray which represents the state vector.
        t : float
            The time variable (scalar)
        v : np.ndarray
            A 1D ndarray which represents any vector of the dimension of u
        args: Any
            The optional arguments for `GopFunction`.

        Returns
        -------
        complex
            G(u, t)v (in quantum mechanics, G(u, t) = -iH(u, t)/hbar). The
            optional arguments included in args will be written in the place of *args in
            the current function, separated by commas.
        """


class GdiffFunction(Protocol):
    def __call__(
        self,
        u1: np.ndarray,
        t1: np.ndarray,
        ...
        """A function object evaluating differential of the generator function
        ...
        u2 : np.ndarray
            1D ndarray; represents the state vector in a particular time-point.
        t2 : float
            scalar; represents the time-point in which u2 is evaluated.
        args: Any
            As above; the optional arguments must be the same as for `GopFunction`.

        Returns
        -------
        np.ndarray
            2D ndarray, where the j'th column represents
...
class SemiGloabalMaxError(TypedDict):
    """A nested dictionary which contains the maximal estimated errors;
    contains the following keys

    Parameters
    ----------
    texp: float
        The maximal estimated error of U, resulting from the time-expansions in
        each time-step
    fU: float
        The maximal estimated error of U, resulting from the computation of
        the function of matrix
    f: float
        The maximal estimated error of the function of matrix
        computation itself
    conv: float
        The maximal estimated convergence error
    """
...
class SemiGloabalHistory(TypedDict):
    """A dictionary with the details of the propagation process

    Parameters
    ----------
    mniter: float
        The mean number of iteration for a time step, excluding the first step,
        which typically requires more iterations. Usually, mniter should be 1 for ideal
        efficiency of the algorithm.
    matvecs: int
        The number of G(u, t) operations on a vector
    t: np.ndarray
        1D ndarray; the time grid of propagation
    U: np.ndarray
        2D ndarray; the solution at t, where different time-points are represented
        by separate columns
    Utestp: np.ndarray
        2D ndarray; the solution at all test-points in all time-steps,
        where different time steps are represented by separate columns
    texp_error: np.ndarray
        1D ndarray; the estimations for the error of U resulting from the
        time-expansion, for all time-steps
    f_error: np.ndarray
        1D ndarray; the estimations for the error of the computation of the
    ...
    conv_error: np.ndarray
        1D ndarray; the estimations for the convergence errors for all time-steps
    niter: np.ndarray
        1D ndarray; the number of iterations for each time-steps
    max_errors: SemiGloabalMaxError
        A nested dictionary which contains the maximal estimated errors
    """
...
class SemiGloabalResult(NamedTuple):
    ...
    ----------
    U: np.ndarray
        2D ndarray; contains the solution at the time points specified in tgrid;
        the different time-points are respresented by separate columns.
    history: SemiGloabalHistory
        A dictionary with the details of the propagation process
    """
...
def SemiGlobal(
    ...
    *args,
    ...
    ----------
    Gop: GopFunction
        Dynamics generator function
    Gdiff_op: GdiffFunction
        Differencial of dynamics generator function
    Gdiff_matvecs: int
        The number of matrix-vector multiplications required for
    ...
    history = SemiGloabalHistory(max_errors={})
    ...
    if not Arnoldi:
        ...
        Ccheb_f_comp = f_chebC(
            t_2ts[1 : (2 * Nt_ts + 1)], Nfm, Nt_ts, min_ev, max_ev, tol, factorialNt_ts
        )
        ...
        f_error = np.max(
            np.abs(chebc2result(Ccheb_f_ts[:, Nt_ts - 2], ev_domain, ztest) - fztest)
            / np.abs(fztest)
        )
        history["max_errors"]["f"] = f_error
        if f_error > 1e-5:
            print(
                f"Warning: The estimated error of the computation of the function of matrix ({f_error}) is larger than 1e-5.\nInstability in the propagation process may occur."
            )
    if not save_memory:
    ...
    history["t"] = np.delete(
        propagation_grid, np.r_[(Nt_ts - 1) : (Nts * Nt_ts) : Nt_ts]
    )
    ...
    for tsi in range(0, Nts):
        ...
        t = propagation_grid[
            tsi * Nt_ts + np.r_[np.r_[0 : (Nt_ts - 1)], Nt_ts, Nt_ts - 1]
        ]
        ...
        niter = 0
        reldif = tol + 1
        while reldif > tol and (
            (tsi > 0 and niter < Niter) or (tsi == 0 and niter < Niter1st)
        ):
            # Calculation of the inhomogeneous s_ext vectors. Note that
            # s_ext[:, tmidi] is equivalent to s[:, tmidi], and therefore not
            # calculated.
            s_ext[:, s_ext_i] = Gdiff_op(
                Ulast[:, s_ext_i], t[s_ext_i], Ulast[:, tmidi], t[tmidi], *args
            )
            ...
            for polyi in range(1, Nt_ts + 1):
                v_vecs[:, polyi] = (
                    Gop(Ulast[:, tmidi], t[tmidi], v_vecs[:, polyi - 1], *args)
                    + Ctaylor[:, polyi - 1]
                ) / polyi
            ...
            if Arnoldi:
                ...
                Upsilon, Hessenberg = createKrop(
                    lambda v: Gop(Ulast[:, tmidi], t[tmidi], v, *args),
                    v_vecs[:, Nt_ts],
                    Nfm,
                    data_type,
                )
                ...
                Unew[:, 1 : (Nt_ts + 1)], f_error, fUerror = Ufrom_vArnoldi(
                    v_vecs[:, 0:Nt_ts],
                    timeMts,
                    Upsilon[:, 0:Nfm],
                    RvKr,
                ...
                # within the time step:
                Unew[:, 1 : (Nt_ts + 1)], fUerror = Ufrom_vCheb(
                    v_vecs[:, 0:Nt_ts], timeMts, Vcheb, Ccheb_f_ts, f_error
                )
            ...
            s_ext_testpoint = dvd2fun(
                (4 / Tts) * t_ts, Cnewton, (4 / Tts) * test_tpoint
            )
            # The error estimation of the time-expansion:
            texpansion_error = (
                norm(s_ext_testpoint - s_ext[:, Nt_ts])
                * np.abs(Tts)
                / norm(Unew[:, Nt_ts])
            )
            # Convergence check of u:
            reldif = norm(Unew[:, Nt_ts - 1] - Ulast[:, Nt_ts - 1]) / norm(
                Ulast[:, Nt_ts - 1]
            )
            Ulast = Unew.copy()
            niter += 1
        if display_mode and reldif > tol:
            print(
                f"Warning: The program has failed to achieve the desired tolerance in the iterative process (in time step No. {tsi + 1})."
            )
            # In such a case, change Nts, Nt_ts and/or Nfm.
        if display_mode and texpansion_error > tol:
            print(
                f"Warning: The estimated error of the time expansion ({texpansion_error}) is larger than the requested tolerance.\nThe solution might be inaccurate (in time step No. {tsi + 1})."
            )
        if display_mode and fUerror > tol:
            print(
                f"Warning: The estimation of the error resulting from the function of matrix ({fUerror}) is larger than the requested tolerance.\nThe solution might be inaccurate (in time step No. {tsi + 1})."
            )
        if display_mode and Arnoldi and f_error > 1e-5:
            print(
                f"Warning: The estimated error of the function of matrix ({f_error}) is larger than 1e-5.\nInstability in the propagation process may occur (in time step No. {tsi + 1})."
            )
        ...
        while (
            tgrid_upi < (Nt - 1)
            and (t[Nt_ts - 1] - tgrid[tgrid_upi + 1]) * direction
            > np.spacing(np.abs(t[Nt_ts - 1])) * 10
        ):
        ...
        if tgrid_lowi <= tgrid_upi:
            ...
            if Arnoldi:
                U[:, tgrid_lowi : (tgrid_upi + 1)] = Ufrom_vArnoldi(
                    v_vecs[:, 0:Nt_ts],
                    timeMout,
                    Upsilon[:, 0:Nfm],
                    RvKr,
                ...
                Ccheb_f_out = f_chebC(
                    tgrid[tgrid_lowi : (tgrid_upi + 1)] - t[0],
                    Nfm,
                    Nt_ts,
                    min_ev,
                    max_ev,
                ...
                U[:, tgrid_lowi : (tgrid_upi + 1)] = Ufrom_vCheb(
                    v_vecs[:, 0:Nt_ts], timeMout, Vcheb, Ccheb_f_out
                )
            tgrid_lowi = tgrid_upi + 1
        # If one of the points in tgrid coincides with the point of the
        # propagation grid:
        if (
            np.abs(t[Nt_ts - 1] - tgrid[tgrid_upi + 1])
            <= np.spacing(np.abs(t[Nt_ts - 1])) * 10
        ):
            tgrid_upi = tgrid_upi + 1
        ...
        if Arnoldi:
            Uguess[:, 1 : (Nt_ts + 1)] = Ufrom_vArnoldi(
                v_vecs[:, 0:Nt_ts],
                timeMnext,
                Upsilon[:, 0:Nfm],
                RvKr,
            ...
            Uguess[:, 1 : (Nt_ts + 1)] = Ufrom_vCheb(
                v_vecs[:, 0:Nt_ts], timeMnext, Vcheb, Ccheb_f_next
            )
        if there_is_ih:
            s[:, 0] = s[:, Nt_ts - 1]
    ...
    if history["max_errors"]["texp"] > tol:
        print(
            f"Warning: The maximal estimated error of the time expansion ({history['max_errors']['texp']}) is larger than the requested tolerance.\nThe solution might be inaccurate."
        )
    if history["max_errors"]["fU"] > tol:
        print(
            f"Warning: The maximal estimated error resulting from the function of matrix computation ({history['max_errors']['fU']}) is larger than the requested tolerance.\nThe solution might be inaccurate."
        )
    if Arnoldi and history["max_errors"]["f"] > 1e-5:
        print(
            f"Warning: The maximal estimated error of the function of matrix ({history['max_errors']['f']}) is larger than 1e-5.\nInstability in the propagation process is possible."
        )
    if history["max_errors"]["conv"] > tol:
        print(
            f"Warning: The maximal estimated error resulting from the iterative process ({history['max_errors']['conv']}) is larger than the requested tolerance.\nThe solution might be inaccurate."
        )
    ...
    return SemiGloabalResult(U, history)
