...
def test_result_harmonic_cheb():
    U, _ = SemiGlobal(Gop, Gdiff_op, 0, fi0_harmonic, np.r_[0, 10], 200, 9, 9, 1e-5,
                      ev_domain=np.r_[-188*1j, 1j], save_memory=True)
    mx = np.sum(np.conj(U[:, 1])*U[:, 1]*x)
    assert np.abs(mx - 2.720105556)<1e-8
    
    
def test_result_harmonic_arnoldi():
    U, _ = SemiGlobal(Gop, Gdiff_op, 0, fi0_harmonic, np.r_[0, 10], 200, 9, 9, 1e-5, save_memory=True)
    mx = np.sum(np.conj(U[:, 1])*U[:, 1]*x)
    assert np.abs(mx - 2.720105556)<1e-8


def test_resultBECcheb():
    U, _ = SemiGlobal(GopBEC, Gdiff_opBEC, 0, gsBEC, np.r_[0, 10], 200, 9, 9, 1e-5,
                      ev_domain=np.r_[-188*1j, 1j], save_memory=True)
    mx = np.sum(np.conj(U[:, 1])*U[:, 1]*x)
    assert np.abs(mx - 2.720105556)<1e-8
...
def test_resultBECarnoldi():
    U, _ = SemiGlobal(GopBEC, Gdiff_opBEC, 0, gsBEC, np.r_[0, 10], 200, 9, 9, 1e-5, save_memory=True)
    mx = np.sum(np.conj(U[:, 1])*U[:, 1]*x)
    assert np.abs(mx - 2.720105556)<1e-8
...
def test_result_ih_cheb():
    U, _ = SemiGlobal(Gop, Gdiff_op, 0, fi0_harmonic, np.r_[0, 10], 200, 9, 9, 1e-5,
                      ihfun, ev_domain=np.r_[-188*1j, 1j], save_memory=True)
    mx = np.sum(np.conj(U[:, 1])*U[:, 1]*x)
